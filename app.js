...
app.post('/check-event-tag', async (req, res) => {
  ...
  try {
    ...
    if (!event) {
      ...
      const notRegisteredCollection = notRegisteredDb.collection('eventtags_notregs');
...
async function validateCredentials(username, password) {
  ...
  try {
    ...
    // Access 'deviceadmin' collection in the database
    const db = client.db('test');
    const deviceAdminCollection = db.collection('deviceadmin');

    // Find the document with the provided username
    const admin = await deviceAdminCollection.findOne({ dev_username: username });

    // If admin is found and passwords match, return true; otherwise, return false
    return admin && admin.dev_password === password;
  } catch (error) {
    // Handle errors
...
app.post('/send-to-database', async (req, res) => {
  try {
    ...
    const { _id, usr_FirstName, usr_LastName, usr_Course, usr_Section, usr_StudentNum, usr_Type } = user;
    ...
    const existingNotDoneData = await notDoneCollection.findOne({ tagId, evt_TagId });

    if (existingNotDoneData) {
      // If existing data found, delete it
      await notDoneCollection.deleteOne({ tagId, evt_TagId });
    }
...
// Retrieve user information from the database
    ...
    const existingData = await checkExistingDataa(tagId, usr_FirstName, usr_LastName, usr_Course, usr_Section, usr_StudentNum, usr_Type, evt_TagId, evt_Title, evt_HostOrg);

    if (existingData) {
      // If the data already exists, send a response indicating it
      return res.status(400).send('Time-in data already exists in the database');
    }

    // Check if the user's time-in data already exists in the 'evpnotdones' collection
    const existingNotDoneData = await checkExistingNotDoneData(tagId, usr_FirstName, usr_LastName, usr_Course, usr_Section, usr_StudentNum, usr_Type, evt_TagId, evt_Title, evt_HostOrg);

    if (existingNotDoneData) {
      // If the data already exists, send a response indicating it
      return res.status(400).send('Time-in data already exists in the not done database');
    }
...
async function checkExistingDataa(tagId, usr_FirstName, usr_LastName, usr_Course, usr_Section, usr_StudentNum, usr_Type, evt_TagId, evt_Title, evt_HostOrg) {
  const client = await connectToDatabase();
  const db = client.db('test');
...
async function checkExistingNotDoneData(tagId, usr_FirstName, usr_LastName, usr_Course, usr_Section, usr_StudentNum, usr_Type, evt_TagId, evt_Title, evt_HostOrg) {
  const client = await connectToDatabase();
  const db = client.db('test');
